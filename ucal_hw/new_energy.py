...
class Monochromator(DeadbandMixin, PVPositioner):
    ...
    Scan_Stop = Cpt(EpicsSignal,":ENERGY_ST_CMD.PROC",name="MONO scan stop command",kind="normal")
...
class EnPos(PseudoPositioner):
    ...
    epugap = Cpt(UndulatorMotor, "SR:C07-ID:G1A{SST1:1-Ax:Gap}-Mtr",
                 kind="normal", name="EPU Gap")
    epuphase = Cpt(UndulatorMotor, "SR:C07-ID:G1A{SST1:1-Ax:Phase}-Mtr",
                   kind="normal", name="EPU Phase")
    ...
    def setup_flyscan(self, start_en, stop_en, speed_en):
        yield from bps.abs_set(self.monoen.Scan_Start_ev, start_en)
        yield from bps.abs_set(self.monoen.Scan_Stop_ev, stop_en)
        yield from bps.abs_set(self.monoen.Scan_Speed_ev, speed_en)
        yield from bps.mv(self.energy, start_en)
        self._last_mono_value = start_en
    ...
    def kickoff(self):
        ...
        if not self.ready_to_fly:
            kickoff_st.set_finished(success=False)
            return kickoff_st
        ...
        self.monoen.Scan_Start.set(1).wait()
